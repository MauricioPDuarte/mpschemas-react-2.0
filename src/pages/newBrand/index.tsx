...
interface BrandDevide {
  id: number;
  name: string;
}

export function NewBrand() {
  const [typeDevice, setTypeDevice] = useState<BrandDevide>({} as BrandDevide);
  ...
  async function findTypeDevice() {
   ...
   async function submit(values: BrandDevide) {
    const valuesJson = JSON.stringify(values);



    if(values.id) {
      api.put(`/brand/${id}`, valuesJson, {
        headers: {
      ...
      <Container maxW={1440}>
        ...
        <Card padding={10} backgroundColor='white'>
          ...
          <Formik
            ...
            validate={values => {
              const errors: BrandDevide = {} as BrandDevide;
              if (!values.name) {
